...
| **네이티브 앱** | 이 스킬은 **웹** 전용. 네이티브/하이브리드 앱은 **MACAG 2.0 + MA 인증** → 자매 스킬 **`macag`**(`/macag`) |
...
| 🔗 **`macag` 스킬** (별도) | **네이티브/하이브리드 모바일 앱** — MACAG 2.0 18항목·네이티브 초점 API·TalkBack 심사·MA 인증. 웹이 아니면 `/macag` |
